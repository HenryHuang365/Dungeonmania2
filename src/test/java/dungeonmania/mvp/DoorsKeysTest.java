...
public class DoorsKeysTest {
    ...
    public void pickUpSunStone() {
        DungeonManiaController dmc;
        dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame("d_DoorsKeysTest_sunstone", "c_DoorsKeysTest_pickUpKey");
    ...
    public void useSunStoneWalkThroughOpenDoor() {
        ...
        Position oldpos = TestUtils.getEntities(res, "player").get(0).getPosition();
        //open the door, sunstone is not consumed
        res = dmc.tick(Direction.DOWN);
        assertEquals(1, TestUtils.getInventory(res, "sun_stone").size());
        //player change the position since the door is opened
        assertNotEquals(oldpos, TestUtils.getEntities(res, "player").get(0).getPosition());
